...
/**
 * VirGLRenderer3Component - Combined renderer using VirGLRenderer 1.3.0 core
 * with Mesa 24.3.4 gallium improvements (TGSI, format handling, cso_cache).
 *
 * This version pairs the stable VirGLRenderer 1.3.0 vrend host renderer
 * with updated gallium auxiliary code from Mesa 24.3.4 for better shader
 * translation, format support, and bug fixes.
 *
 * Loads libvirglrenderer3.so.
 */
public class VirGLRenderer3Component extends EnvironmentComponent implements ConnectionHandler, RequestHandler {
    private static final String TAG = "VirGLRenderer3Component";
    ...
    static {
        System.loadLibrary("virglrenderer3");
    }

    public VirGLRenderer3Component(XServer xServer, UnixSocketConfig socketConfig) {
    ...
    @Override
    public void start() {
        Log.d(TAG, "Starting VirGL 24.3.4 renderer...");
    ...
    @Override
    public void stop() {
        Log.d(TAG, "Stopping VirGL 24.3.4 renderer...");
        stopped = true;
        if (connector != null) {
            connector.stop();
            connector = null;
